...
/// A deliberately tiny, one-shot HTTP listener for OAuth loopback redirects.
/// It binds only to 127.0.0.1, accepts no body, caps headers at 8 KiB, and
/// never records the authorization code or query string.
public final class PlatformOAuthCallbackListener: OAuthCallbackListening, @unchecked Sendable {
    ...
    private func writeResponse(to client: Int32, status: String, body: String) {
        ...
        _ = bytes.withUnsafeBytes { buffer in Darwin.send(client, buffer.baseAddress, buffer.count, 0) }
    }
}
#elseif !os(Linux) && !os(Windows)
/// This declaration keeps the package buildable on every supported platform.
/// The platform adapters are supplied in their OS-specific source files.
public final class PlatformOAuthCallbackListener: OAuthCallbackListening, @unchecked Sendable {
    public let redirectURI: String
    public init(timeoutSeconds: TimeInterval = 300, callbackPath: String = "/oauth/callback") throws {
        redirectURI = "http://127.0.0.1:0\(callbackPath)"
        throw OAuthError.credentialStoreUnavailable("OAuth loopback listener is unavailable on this platform build")
    }
    public func waitForCallback() async throws -> URL { throw OAuthError.callbackRejected }
    public func close() {}
}
#endif
